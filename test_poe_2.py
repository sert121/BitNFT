            ...
            if (transaction['address'] == receiveAddress):
                ...
                if (transaction['amount'] >= session.txnAmount):
                    ...
                    with st.spinner('Wait for it...'):
                        received = True
                    st.success("Money received. Waiting for block confirmation.")
        ...
        while True:
            ...
            if tx:
                ...
                if ticks == 60:
                    ...
                    st.write("10 min passed. No money received")
                    break
                else:
                    time.sleep(10)
...
with st.beta_container():
    ...
    st.markdown("<p style='text-align: center; color: white;font:Courier New;'>An online medium to allow hassle-free archiving of files on the BTC Testnet. </p>",unsafe_allow_html=True)
st.markdown('''---''')
st.text("\n\n")
...
with col1:
    ...
    if uploaded_file:
        if st.button("Process"):
            ...
            # receiveAddress = "tb1qsng0zzkczmmlugwrj5l4zyahnt2kmqgsxkxhqf"     # Test Address
...
with col2:
    ...
    if st.button("Verify"):
        ...
        if (result):
            st.success("You are the true owner of this file. This file is on the bitcoin ledger.")
            st.balloons()
        else:
            st.error("RED ALERT!!")
